...
public class X509ShellUtils {
    ...
    public static String build_list_crl(String crl_file) {
        ...
        String command = null;
        if (OSInfo.getOSInfo().isWin()) {
            command = StringContext.systemPath + "/commands/windows/no_pass/build-list-crl.bat " +
                    crl_file;
        } else {
            command = StringContext.systemPath + "/commands/liunx/no_pass/build-list-crl.sh " +
                    crl_file;
        }
        proc.exec(command);
    ...
    //校验证书签名(多个ca需要cat)
    public static String build_verify(String cat_ca_file, String check_certificate) {
        Proc proc = new Proc();
        String command = null;
        if (OSInfo.getOSInfo().isWin()) {
